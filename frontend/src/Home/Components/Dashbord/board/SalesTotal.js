...
export default function SalesTotal() {
  ...
  const sumAllSales  = Object.values(sales).reduce((total, monthData) => total + monthData, 0).toFixed(2); 
  ...
  return (
    ...
    >
      <Paper  elevation={3} style={{display:"flex",flexDirection:"column",justifyContent:"center",alignItems:"center",}} >
        ...
        <h6>Total </h6>
        <span style={{marginTop:"-1.8rem", textDecoration:"underline"}} >€{sumAllSales}</span>
      </Paper >
      <Paper elevation={3} style={{display:"flex",flexDirection:"column",justifyContent:"center",alignItems:"center"}} >
       <VisibilityTwoToneIcon style={{marginBottom:"-1.5rem"}}/>
      <h6>Visitors </h6>
        	
        <span style={{marginTop:"-1.8rem", textDecoration:"underline"}}>{visitors && visitors.counter}</span>
      </Paper>
